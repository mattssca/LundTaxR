...
#' @param these_samples_metadata Required parameter if `this_data`is not provided. Metadata 
#' associated with he prediction output. Also possible for the user to provide a metadata subset 
#' with samples of interest, the return will be restricted to the samples within the specified group
#' @param this_data Required parameter if `these_predictions` and `these_samples_metadata` is not 
#' provided. Should be the output from `get_survival` or `get_glm`, depending on the plot type set 
#' with `stat_plot`.
...
#' @param this_subtype Optional Specify subtype of interest. Leave as NULL to not separate 
#' statistics on subtype.
...
#' @param col_bon Boolean parameter, if set to TRUE the p values will be bonferroni adjusted. 
#' Default is FALSE.
...
#' @param categorical_factor Required parameter if `this_glm`is not provided. This should be the 
#' categorical variable that is intended for testing. In addition, this should also be a variable of 
...
#' @param plot_title Optional parameter, set title for the plot. Default is "My Plot".
#' @param plot_subtitle Optional parameter, set subtitle for the plot. Default is "".
#' @param plot_width This parameter controls the width in inches.  Default is 8 (2400 pixels at 300 
#' PPI).
#' @param plot_height This parameter controls the height in inches. Default is 8 (2400 pixels at 300
#' PPI).
#' @param plot_order Optional parameter for setting the order of scores in the returned plot. Only 
#' applies if `arrange_plot` is set to TRUE.
